-- 0026_real_ole_miss_chapters.sql
-- Replace the GENERIC placeholder chapters that 0025 seeded for ACCY 201 with Lee's REAL
...
--
-- SELF-CONTAINED: produces the correct end state whether or not 0025 was applied. It
...
-- ---------------------------------------------------------------------------
-- 1. Ensure the link column(s) exist (works even if 0025 never ran)
-- ---------------------------------------------------------------------------
...
--    already exists (e.g. 0025 created accy-201 as "Principles of Accounting I"); insert
...
--    (course, number): rename existing rows (converts 0025 placeholders in place),
...
-- ---------------------------------------------------------------------------
-- 5. Re-assert the v2 UI flags on merch-sale (idempotent; covers the case where 0025
...
--    With the real 201 map covering 1-11, step 3 renames every 0025 placeholder, so this
