...
export async function sendResendEmail(opts: { to: string; subject: string; text: string }): Promise<EmailResult> {
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({ from: FROM, to: [to], reply_to: REPLY_TO, subject: opts.subject, text: opts.text }),
    });
    const j = (await res.json().catch(() => ({}))) as { id?: string };
    if (!res.ok) return { ok: false, error: `Resend ${res.status}: ${JSON.stringify(j).slice(0, 200)}` };
