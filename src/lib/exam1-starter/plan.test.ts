...
// workbook so the canonical contract (6/25/280 + structural invariants) is CI-enforced.
import { test, expect } from "bun:test";
import { join } from "node:path";
import { readImportRows } from "./workbook";
import { buildPlan, deckIdFor, ceqIdFor, EXPECTED } from "./plan";

const WORKBOOK = join(import.meta.dir, "..", "..", "..", "scripts", "curriculum", "Survive_Exam1_Global_Starter_Map.xlsx");
...
test("canonical counts: 6 topics / 25 sets / 280 CEQs", () => {
...
test("every set belongs to exactly one topic; every CEQ to exactly one set", () => {
  ...
  expect(setToTopic.size).toBe(25);
...
test("no duplicate CEQ ids or Question Keys; no dup prompt within a set", () => {
  ...
  expect(ids.size).toBe(280);
});

test("205 reused + 75 new; deterministic ids are stable across two builds", () => {
  const reused = plan.sets.reduce((a, s) => a + s.ceqs.filter((c) => c.reused).length, 0);
  expect(reused).toBe(205);
  expect(plan.ceqCount - reused).toBe(75);
