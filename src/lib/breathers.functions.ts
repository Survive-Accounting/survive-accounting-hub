...
type Pub = { kind?: string; source?: string; state?: string; pubKey?: string; takeIndex?: number; order?: number; hidden?: boolean; meta?: { title?: string }; render?: { durationS?: number | null; muxPlaybackId?: string }; coverUrl?: string };
...
export interface SequenceVideo { pubKey: string; index: number; takeIndex: number; title: string; durationS: number | null; coverUrl: string | null; playbackId: string | null; hidden: boolean }
...
export const loadBreathers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<{ setName: string; videos: SequenceVideo[]; breathers: Breather[] } | null> => {
    ...
    const videos = (d.publications ?? []).filter(isPosted).sort((a, b) => siteOrder(a) - siteOrder(b) || (a.takeIndex ?? 0) - (b.takeIndex ?? 0))
      .map((p, i) => ({ pubKey: p.pubKey!, index: i, takeIndex: p.takeIndex ?? 0, title: p.meta?.title ?? `Video ${i + 1}`, durationS: p.render?.durationS ?? null, coverUrl: p.coverUrl ?? null, playbackId: p.render?.muxPlaybackId ?? null, hidden: p.hidden === true }));
