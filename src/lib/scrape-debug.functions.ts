...
export const listImprovementSuggestions = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: rows, error } = await supabaseAdmin
      ...
      .select("id,bundle_id,campus_id,campus_name,model,pattern_tag,severity,title,suggestion,created_at")
      .order("created_at", { ascending: false })
      .limit(data.limit);
    if (error) throw new Error(error.message);
    ...
    const byTag = new Map<string, { tag: string; count: number; severity: string; latest: Row; campuses: Set<string> }>();
    for (const r of (rows ?? []) as Row[]) {
      ...
      if (existing) {
        ...
        byTag.set(tag, { tag, count: 1, severity: r.severity ?? "low", latest: r, campuses });
      }
    }
