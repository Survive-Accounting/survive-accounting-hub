  ...
  return (
    <div style={wrap}>
      ...
      <main style={{ position: "relative", zIndex: 1, maxWidth: 760, margin: "0 auto", paddingLeft: 20, paddingRight: 20, width: "100%" }} className="pb-20 sm:pb-28">
        ...
        {/* PRE-ONBOARDING (2026-09-06) lives on its own page, before approval: a verified applicant
            who hasn't finished it goes there; one who has waits there for Lee's text. */}
        {st.s === "ready" && st.d.applicationStatus === "setup" && <GoOnboarding k={k ?? null} />}
        {st.s === "ready" && st.d.applicationStatus === "submitted" && (
          <div className="mx-auto max-w-sm pt-16 text-center" style={{ fontFamily: BRAND_SANS }}>
            ...
            <h1 className="mt-2 text-[22px] font-black" style={{ fontFamily: BRAND_DISPLAY }}>Your onboarding is in.</h1>
            <p className="mt-2 text-[14px]" style={{ color: "var(--text-muted)" }}>Lee gets it on his phone. Expect a text within two business days to set up a quick call; your dashboard opens the moment you're approved after it.</p>
            <a href={k ? `/rep/onboarding?k=${encodeURIComponent(k)}` : "/rep/onboarding"} className="mt-4 inline-flex items-center rounded-xl px-4 text-[13.5px] font-black" style={{ minHeight: 44, background: "var(--bg-overlay)", border: "1px solid var(--border-default)", color: "var(--brand-cream)" }}>See where it stands →</a>
...
/** A verified applicant who hasn't finished the pre-onboarding: the workspace isn't theirs yet,
 *  the onboarding is. Full navigation (not the router) so the `k` test token survives. */
function GoOnboarding({ k }: { k: string | null }) {
  const href = k ? `/rep/onboarding?k=${encodeURIComponent(k)}` : "/rep/onboarding";
  useEffect(() => { window.location.assign(href); }, [href]);
  return (
    <div className="mx-auto max-w-sm pt-16 text-center" style={{ fontFamily: BRAND_SANS }}>
      <p className="text-[14px]" style={{ color: "var(--text-muted)" }}>Your dashboard opens after the onboarding. Taking you there…</p>
      <a href={href} className="mt-3 inline-block font-bold underline underline-offset-4" style={{ color: "var(--accent)" }}>Open the onboarding →</a>
    </div>
  );
}
