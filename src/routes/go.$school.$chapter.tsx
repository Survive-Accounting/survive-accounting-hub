...
function GoChapterPage() {
  ...
  const onAction = (action: "open_learn" | "copy_link" | "copy_groupme" | "flyer" | "slide") => {
    ...
    // share_copied (lib/analytics → the ad tags): the two copy actions, campus + chapter only.
    if (action === "copy_link" || action === "copy_groupme") track("share_link_copied", { campus_slug: school, chapter_slug: chapter, source: ev.via });
  };
