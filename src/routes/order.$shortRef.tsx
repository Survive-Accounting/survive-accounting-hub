// /order/$shortRef — student Custom Study Pack tracker. Gated by an email
...
export const Route = createFileRoute("/order/$shortRef")({
  head: () => ({
    meta: [
      { title: "Track your Custom Study Pack — Survive Accounting" },
  ...
  return (
    <div className="rounded-3xl bg-white p-6 shadow-[0_10px_40px_-15px_rgba(20,33,61,0.15)] sm:p-9">
      {sent ? (
        ...
        <>
          <h1 className="text-xl font-bold sm:text-2xl" style={{ color: NAVY }}>Track your Custom Study Pack</h1>
          {expired && (
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl bg-white p-6 shadow-[0_10px_40px_-15px_rgba(20,33,61,0.15)] sm:p-8">
        <h1 className="text-xl font-bold sm:text-2xl" style={{ color: NAVY }}>Custom Study Pack #{o.short_ref}</h1>
        ...
        <div className="mt-5 rounded-2xl border bg-gray-50 p-4">
          ...
          <div className="mt-3 border-t border-dashed border-gray-300 pt-3 space-y-1.5">
            ...
            <Row label="PAYMENT" value="Pay only if you unlock the full pack" />
                    ...
                    <div className="mt-2">
                      {latestFor(["unlocked", "preview_ready"])?.unlock_url ? (
                        <a href={latestFor(["unlocked", "preview_ready"])!.unlock_url!} className="inline-block rounded-lg px-4 py-2 text-sm font-bold text-white" style={{ background: RED }}>
                          Unlock full study pack — {money(o.unlock_price_cents)}
                        </a>
                      ) : (
                        <p className="rounded-lg bg-gray-50 px-3 py-2 text-sm text-gray-700">Want to unlock the full pack? Text Lee at {WORK_PHONE}.</p>
                      )}
                    </div>
                  )}
                </div>
      ...
      <p className="px-2 text-center text-xs text-gray-500">
        Custom Study Packs are designed to supplement your studying, not replace your class notes, homework, textbook, or professor’s materials.
      </p>
    </div>
  );
}
...
function HelpSection({ shortRef, order, onReload }: { shortRef: string; order: StudentOrder; onReload: () => void }) {
  ...
  return (
    <div className="rounded-3xl bg-white p-6 shadow-[0_10px_40px_-15px_rgba(20,33,61,0.15)] sm:p-8">
      <h2 className="text-lg font-bold" style={{ color: NAVY }}>Help me build the right pack</h2>
