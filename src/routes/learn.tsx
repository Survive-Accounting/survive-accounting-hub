...
type LearnSearch = {
  ...
  // forwarded the link (sharer; shows the vouched banner). g = the campus slug the /s/<campus>
  // hop resolved, so the CTA bar can scope its copy before a chapter is picked. test = force a CTA
  // state (A–F) or the banner, client-side, no DB — mirrors ?demo=1.
...
export const Route = createFileRoute("/learn")({
  validateSearch: (s: Record<string, unknown>): LearnSearch => ({
    ...
    // GREEK SHARE FUNNEL — ref/by are contact UUIDs (anything else dropped, so a rep code can never
    // be mistaken for a contact); g is a campus slug; test is a single CTA-state letter (or "banner").
