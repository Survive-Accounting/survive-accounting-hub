  ...
  const playerItems = useMemo<PlayerItem[]>(() => {
    ...
    for (const s of sets) {
      ...
      const list: PlayerPart[] = shorts
        ? shorts.map((sh, i) => ({ index: i, of: shorts.length, name: sh.name, playbackId: sh.playbackId, coverUrl: sh.coverUrl, key: partKey(s.set.id, i), endCta: sh.endCta ?? null, gate: sh.gate ?? null }))
