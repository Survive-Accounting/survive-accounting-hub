...
import { Landmark, GraduationCap, Loader2, CheckCircle2, XCircle, X, ShieldAlert, Ban, ChevronDown } from "lucide-react";
      ...
      {jobs.length > 0 && (
        <Collapsible open={queueOpen} onOpenChange={setQueueOpen} className="px-1">
          <div className="flex items-center justify-between pb-1">
            <CollapsibleTrigger className="flex flex-1 items-center gap-1 rounded px-1 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sidebar-foreground/60 hover:bg-sidebar-accent">
              <ChevronDown className={`h-3 w-3 transition-transform ${queueOpen ? "rotate-180" : ""}`} />
              <span>Scrape Queue</span>
              <span className="ml-1 font-mono tabular-nums text-sidebar-foreground/50">{jobs.length}</span>
