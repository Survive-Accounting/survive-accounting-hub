...
export function LearnRail({ active, onPick, expanded, onToggle, path, activeSetId, onOpenSet }: {
  ...
  return (
    <aside className="flex shrink-0 flex-col" style={{ width: expanded ? 300 : 80, transition: "width 160ms ease", background: LK.bg, padding: "12px 8px", position: "sticky", top: 0, height: "100%", overflowY: "auto", scrollbarWidth: "none" }}>
      <button type="button" onClick={onToggle} className="mb-2 grid h-10 w-10 place-items-center rounded-lg" style={{ background: "transparent", border: 0, color: LK.text, cursor: "pointer" }} aria-label={expanded ? "Collapse" : "Your path"} title={expanded ? "Collapse" : "Your path"}>
...
export function PathList({ path, activeSetId, onOpenSet }: { path: PathTopic[]; activeSetId: string | null; onOpenSet: (setId: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-1 px-1">
      <div className="px-2 pb-1 text-[10.5px] font-extrabold uppercase" style={{ letterSpacing: "0.14em", color: LK.muted }}>Your path</div>
      {path.map(({ topic, sets, done }) => {
        const all = sets.length > 0 && done === sets.length;
        const o = isOpen(topic.id);
        return (
          <div key={topic.id}>
            <button type="button" onClick={() => setOpen((m) => ({ ...m, [topic.id]: !o }))} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left" style={{ background: o ? LK.surface : "transparent", border: 0, color: LK.text, cursor: "pointer" }}>
              {o ? <ChevronDown className="h-3.5 w-3.5 shrink-0" style={{ color: LK.muted }} /> : <ChevronRight className="h-3.5 w-3.5 shrink-0" style={{ color: LK.muted }} />}
              <span className="min-w-0 flex-1 truncate text-[13px] font-bold">{topic.name}</span>
              <span className="shrink-0 text-[11px] tabular-nums" style={{ color: all ? LK.green : LK.muted }}>{all ? <Check className="h-3.5 w-3.5" /> : done > 0 ? `${done}/${sets.length}` : sets.length}</span>
            </button>
            {o && (
              <div className="ml-4 flex flex-col border-l py-1 pl-3" style={{ borderColor: LK.border }}>
                {sets.map(({ set, done: d, playable, locked }) => {
                  const on = set.id === activeSetId;
                  return (
                    <button key={set.id} type="button" disabled={!playable && !locked} onClick={() => onOpenSet(set.id)} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-left disabled:cursor-default" style={{ background: "transparent", border: 0, color: on ? "var(--lk-acc)" : playable || locked ? LK.text : LK.dim, cursor: "pointer" }}>
                      {locked ? <Lock className="h-3 w-3 shrink-0" style={{ color: LK.muted }} /> : d ? <Check className="h-3 w-3 shrink-0" style={{ color: LK.green }} /> : <Circle className="h-2.5 w-2.5 shrink-0" style={{ color: on ? "var(--lk-acc)" : LK.dim }} />}
                      <span className="min-w-0 flex-1 truncate text-[12.5px]" style={{ fontWeight: on ? 700 : 500 }}>{set.shortLabel || set.name}</span>
                      {!playable && !locked && <span className="text-[10px]" style={{ color: LK.dim }}>soon</span>}
                    </button>
                  );
                })}
              </div>
...
export function LearnTabs({ active, onPick }: { active: RailKey; onPick: (k: RailKey) => void }) {
  return (
    <nav className="flex shrink-0 px-1 pb-3 pt-2" style={{ background: LK.bg, borderTop: `1px solid ${LK.border}` }}>
      {RAIL_ITEMS.filter((i) => i.key !== "review").map((it) => (
        <button key={it.key} type="button" onClick={() => onPick(it.key)} className="flex flex-1 flex-col items-center gap-0.5 py-1 text-[10px] font-semibold" style={{ background: "transparent", border: 0, color: active === it.key ? LK.text : LK.muted, cursor: "pointer" }}>
          <RailIcon k={it.key} on={active === it.key} />
          {it.label}
        </button>
      ))}
