    ...
    >
      ...
      >
        ...
        <span className={`flex items-center gap-0.5 transition-opacity ${showChrome ? "opacity-100" : "pointer-events-none opacity-0"}`}>
          <button className={btn} style={{ color: nav.canStep(id, -1) ? NEON.text : NEON.borderSoft }} title="Previous frame" disabled={!nav.canStep(id, -1)} onPointerDown={stop} onClick={(e) => { e.stopPropagation(); nav.enter(id); nav.step(-1); }}><ChevronLeft className="h-3.5 w-3.5" /></button>
          <button className={btn} style={{ color: nav.canStep(id, 1) ? NEON.text : NEON.borderSoft }} title="Next frame" disabled={!nav.canStep(id, 1)} onPointerDown={stop} onClick={(e) => { e.stopPropagation(); nav.enter(id); nav.step(1); }}><ChevronRight className="h-3.5 w-3.5" /></button>
