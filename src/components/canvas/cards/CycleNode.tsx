// ACCOUNTING CYCLE (Lee) — a raised callout box (exam-cue vibe) whose steps sit
// evenly around an OVAL, joined by clockwise flow arrows that close the loop.
// Add / remove / rename steps; the oval re-solves from the step count so it's
// always an even ring. Design ELEMENT: resizable, spotlightable (whole-element
// "self" target, live in film), never in the deck.
...
export function CycleNode({ id, data, selected }: NodeProps) {
  ...
  const pad = seg * 0.2; // gap at each pill so arrows don't tuck under them
  ...
  return (
    ...
    >
      ...
      {/* GRAB HANDLE — the whole box drags too */}
      <div
      ...
      <div
        ...
        style={{
          width: "100%",
          height: h,
          background: "radial-gradient(ellipse at 50% 42%, rgba(30,42,74,0.9), rgba(9,13,26,0.94))",
          border: "1.5px solid rgba(252,163,17,0.4)",
          boxShadow: "0 18px 44px -18px rgba(0,0,0,0.7)",
          ...spotStyle(spot.state),
        }}
      >
        {/* flow arrows (behind the pills) */}
        <svg
        ...
        >
          <defs>
            ...
            <marker id={`cyc-arrow-${id}`} viewBox="0 0 10 10" refX="7" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
        ...
        {/* the step pills, around the oval */}
        {placed.map((s, i) => (
          <div
            key={s.id}
          ...
          >
            {editingStep === s.id ? (
              ...
              >
                <span
                  className="grid h-4 w-4 shrink-0 place-items-center rounded-full text-[8px] font-black"
                  style={{ background: "#FCA311", color: "#0B0F1E" }}
                >
                  {i + 1}
                </span>
                {s.text || "Step"}
