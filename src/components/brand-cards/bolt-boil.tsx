...
// branded intro/outro cards. A FIXED 4-frame cycle (never per-render random) derived from
// the canonical brand bolt: interior vertices wobble <=1.3px and the outline stroke breathes
// 1.8->2.4px between frames; the shared TIP (62,4) and BASE (32,130) are pinned so the bolt
// never drifts as the letter "i". The whole layer is never translated/scaled/rotated. Cycles
// on ~8fps via a pure-CSS discrete swap (no JS timer, capture-friendly). prefers-reduced-
// motion serves a single static frame. React only, no external deps.
...
const VIEWBOX = "24 -2 56 138";
const RATIO = 56 / 138;

const FRAMES: { outer: string; seam: string; sw: number }[] = [
...
export function BoltBoil({ height = 130, opacity = 1, red = BRAND_RED, blue = BRAND_BLUE, cream = BRAND_CREAM, className, style }:
  { height?: number; opacity?: number; red?: string; blue?: string; cream?: string; className?: string; style?: CSSProperties }) {
  return (
    <span className={className} style={{ display: "inline-block", width: height * RATIO, height, opacity, ...style }}>
      <style>{BOIL_CSS}</style>
      <svg viewBox={VIEWBOX} width="100%" height="100%" style={{ display: "block", overflow: "visible" }} aria-hidden>
        {FRAMES.map((f, i) => (
          <g key={i} className="sa-boil-f" style={{ animationDelay: `${(-0.125 * i).toFixed(3)}s` }}>
            <path d={f.outer} fill={red} stroke={cream} strokeWidth={f.sw} strokeLinejoin="round" strokeLinecap="round" paintOrder="stroke" />
            <path d={f.seam} fill={blue} />
          </g>
        ))}
      </svg>
    </span>
...
export function SurviveWordmark({ size, cream = BRAND_CREAM, red = BRAND_RED, blue = BRAND_BLUE, style }:
  { size: number; cream?: string; red?: string; blue?: string; style?: CSSProperties }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "baseline", fontFamily: "'Rubik', system-ui, sans-serif", fontWeight: 900, fontSize: size, lineHeight: 1, letterSpacing: "-0.015em", color: cream, whiteSpace: "nowrap", ...style }}>
      surv
      <BoltBoil height={size * 1.14} red={red} blue={blue} cream={cream} style={{ margin: `0 ${(size * -0.008).toFixed(1)}px`, transform: `translateY(${(size * 0.155).toFixed(1)}px)` }} />
      ve
    </span>
  );
}
