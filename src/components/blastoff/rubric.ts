// THE EQUATION RUBRIC — the A = L + E block a `rubric` slide draws (RubricFrame.tsx), as pure
// data and pure rules. No React, no DOM: this file is what the tests pin and what the Editor,
// the film surface and the schema all agree on.
//
// Lee's priority (the Editor session brief, 2026-09-11): "the A = L + E rubric first (he needs
// it on the cram path). Topic Complete, Up Next, Survibes and the skip button come later."
// The layout is the "Rubric tool" in end-of-topic-frames.html — an L: A = L + E across the top,
// Rev then Exp hanging under E, the transaction in the open space to their left.
//
// THE RULES
//  · A box holds a LIST of arrows: [] · [up] · [down] · [up, down]. Clicking cycles them in that
//    order. "up" is amber, "down" is light blue (the frame's job, not this file's).
//  · EQUITY EFFECT (off by default — "leave that last one off until you've taught it. It's the
//    'expenses up means equity down' moment"): when on and E has no arrows of its own, E shows
//    a FADED arrow derived from Rev (Rev↑ → E↑) and Exp (Exp↑ → E↓).
...
//    A box with both arrows nets to zero (bought supplies with cash: A↑↓).
...
export type RubricArrow = "up" | "down";
...
/** The click cycle on a box: blank → ↑ → ↓ → ↑↓ → blank. */
export const ARROW_CYCLE: readonly (readonly RubricArrow[])[] = [[], ["up"], ["down"], ["up", "down"]];
...
/** A fresh rubric slide: A = L + E, arrows only, nothing typed, equity effect off. A NEW object
 *  every call — inserts must never share one nested object between two frames. */
export function emptyRubric(): RubricSpec {
  return { mode: "ale", text: "", amount: 0, arrows: emptyArrows(), show: "arrows", equityEffect: false };
}

/** `arrows` is a list, but the four states are what they are: ↑↓ and ↓↑ are the same box. */
...
/** The next state of one box in the click cycle. An unknown list (not one of the four) starts over at blank. */
 ...
 *  Deduplicated, in the order they were found. */
export function derivedEquity(arrows: RubricArrows): RubricArrow[] {
  const out: RubricArrow[] = [];
  for (const d of arrows.Rev) if (!out.includes(d)) out.push(d);
  for (const d of arrows.Exp) { const flipped: RubricArrow = d === "up" ? "down" : "up"; if (!out.includes(flipped)) out.push(flipped); }
  return out;
}

/** What E shows: its own arrows, or — with the equity effect on and E blank — the derived ones,
 *  faded (`ghost`). */
export function equityShown(spec: Pick<RubricSpec, "arrows" | "equityEffect">): { arrows: RubricArrow[]; ghost: boolean } {
  if (spec.arrows.E.length || !spec.equityEffect) return { arrows: spec.arrows.E, ghost: false };
  const d = derivedEquity(spec.arrows);
  return { arrows: d, ghost: d.length > 0 };
}

/** One box's net movement in dollars: up = +amount, down = −amount, both = 0. */
export function netOf(arrows: readonly RubricArrow[], amount: number): number {
  return arrows.reduce((t, d) => t + (d === "up" ? amount : -amount), 0);
}
 ...
 *  the slide, not a posting — the student is meant to see that Rev and Exp already carry it). */
export function balanceDiff(spec: Pick<RubricSpec, "arrows" | "amount">): number {
  const n = (k: RubricKey) => netOf(spec.arrows[k], spec.amount);
  return n("A") - (n("L") + n("E") + n("Rev") - n("Exp"));
}

export const fmtDollars = (n: number): string => `$${Math.round(Math.abs(n)).toLocaleString("en-US")}`;

/** The balance line — null when there is nothing to say (arrows mode, no amount, or no arrows). */
export function balanceLine(spec: Pick<RubricSpec, "arrows" | "amount" | "show">): { ok: boolean; text: string } | null {
  if (spec.show !== "amounts" || !(spec.amount > 0) || !anyArrows(spec.arrows)) return null;
  const diff = balanceDiff(spec);
  return diff === 0 ? { ok: true, text: "⚖ Balanced" } : { ok: false, text: `⚖ Off by ${fmtDollars(diff)}` };
}

/** THE REVEAL ORDER on the film surface: the boxes that have something to show, grouped as
 *  the spacebar walks them — A, then L, then E, then Rev and Exp together (they are one row of
 *  the L). A group with nothing in it is skipped. E counts when it has arrows of its own OR the
 *  equity effect gives it a derived one. */
export function revealGroups(spec: Pick<RubricSpec, "arrows" | "equityEffect">): RubricKey[][] {
  const groups: RubricKey[][] = [];
  if (spec.arrows.A.length) groups.push(["A"]);
  if (spec.arrows.L.length) groups.push(["L"]);
  if (equityShown(spec).arrows.length) groups.push(["E"]);
  if (spec.arrows.Rev.length || spec.arrows.Exp.length) groups.push(["Rev", "Exp"]);
  return groups;
}

/** How many spacebar states the slide has: the bare block, then one per reveal group. */
export function rubricSteps(spec: Pick<RubricSpec, "arrows" | "equityEffect">): number {
  return 1 + revealGroups(spec).length;
}
...
export function revealedKeys(spec: Pick<RubricSpec, "arrows" | "equityEffect">, step: number | undefined): Set<RubricKey> {
  if (step === undefined) return new Set(RUBRIC_KEYS);
  const out = new Set<RubricKey>();
  for (const g of revealGroups(spec).slice(0, Math.max(0, step))) for (const k of g) out.add(k);
  return out;
}
...
/** THE PRESETS — the eight transactions Lee named for the panel, applied on click. Arrows only
 *  where the transaction moves a box; the amount rides along for amounts mode. */
 ...
 *  equity toggle are Lee's and stay. */
export function applyPreset(spec: RubricSpec, p: RubricPreset): RubricSpec {
  ...
  return { ...spec, text: p.text, amount: p.amount, arrows };
}
