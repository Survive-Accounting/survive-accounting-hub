...
import { DC_KEYS, DC_MODE_LABEL, DC_MODES, DC_NAME, DC_ORANGE, DC_YELLOW, dcColor, dcPickOf, dcTypeOf, dcWhenOf, formatTLines, isDcKey, isDcMode, parseTLines, tPickOf } from "./ledger";
  ...
  return (
    <section style={panelShell}>
      ...
      <div>
        ...
        {sel.kind === "ceq" && ceq && !ceq.noteOnly && (dcPickOf(ceq.stem, ceq.choices) || dcWhenOf(ceq.stem, ceq.choices)) && (
          <div className="flex flex-wrap items-center" style={{ gap: 6, fontSize: 11, color: MUTED }}>
            ...
            {(() => { const auto = dcTypeOf((dcPickOf(ceq.stem, ceq.choices) ?? dcWhenOf(ceq.stem, ceq.choices))?.account ?? ""); return <button style={chip(!!sel.dcpick && !isDcKey(sel.dcpick), auto ? dcColor(auto) : GOLD)} onClick={() => onPatch({ dcpick: "auto", tpick: undefined })}>{auto ? `Auto · ${DC_NAME[auto]}` : "Auto"}</button>; })()}
