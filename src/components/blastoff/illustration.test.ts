...
describe("the illustration registry", () => {
  test("the house default is Survive Watercolor v4, white ground, house palette", () => {
  ...
  test("stale = made with an older registry version; never for an ungenerated request", () => {
    ...
    expect(isStaleIllustration({ ...emptyIllustration(), assetUrl: "x", styleVersion: 4 })).toBe(false);
    expect(isStaleIllustration({ ...emptyIllustration(), assetUrl: "x", styleVersion: 3 })).toBe(true);
