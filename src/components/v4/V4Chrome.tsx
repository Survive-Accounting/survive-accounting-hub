...
import { V4_BAR_LABEL, V4_BAR_STEPS, barStepDone, barStepOf, type V4BarStep, type V4StateView } from "./v4-topic";

export const V4_MINT = "#3BF5A0";
export const V4_RED = "#FF8B7E";
export const V4_AMBER = "#F59E0B";

export function V4StepBar({ topicKey, setKey, state, current }: { topicKey: string; setKey: string; state: V4StateView | null; current: V4BarStep }) {
  return (
    <nav aria-label="Topic steps" style={{ display: "flex", gap: 6, flexWrap: "wrap", margin: "12px 0 18px" }}>
      {V4_BAR_STEPS.map((s, i) => {
        const done = barStepDone(s, state?.final ?? {});
        const here = barStepOf(state?.step) === s;
