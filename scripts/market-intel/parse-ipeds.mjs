...
// Prefer revised (_RV) files; 2024 is provisional (no _RV released yet as of 2026-08).
const COMP_YEARS = {
...
// --- Directory ---
console.log('Parsing HD2024 (directory)...');
await eachLine('HD2024.csv', (line) => {
...
// Fallback directory: add any institution present in HD2023 but not in HD2024 (closed/renamed
// in the newer directory) so we don't lose campuses that still carry completions history.
await eachLine('HD2023.csv', (line) => {
  const f = parseCsv(line);
  const u = unq(f[0]);
  if (!u || inst.has(u)) return;
  const r = rec(u);
  r.name = f[1]; r.alias = f[2]; r.city = f[4]; r.state = f[5]; r.directory_year = 2023;
});

// CONTROL & SECTOR need header index — reparse header once (per directory year)
for (const hd of ['HD2024.csv', 'HD2023.csv']) {
  const header = parseCsv(fs.readFileSync(path.join(DIR, hd), 'utf8').split(/\r?\n/)[0]);
  const iControl = header.findIndex((h) => unq(h).toUpperCase() === 'CONTROL');
  const iSector = header.findIndex((h) => unq(h).toUpperCase() === 'SECTOR');
  await eachLine(hd, (line) => {
    const f = parseCsv(line);
    const u = unq(f[0]); if (!inst.has(u)) return;
    const r = inst.get(u);
    if (r.control == null) { r.control = num(f[iControl]); r.sector = num(f[iSector]); }
  });
}
...
// Column layout differs by year (older files lack the EFFYALEV column), so resolve
// EFFYLEV + EFYTOTLT indices from each file's own header.
for (const [year, file] of [[2018, 'effy2018_rv.csv'], [2019, 'effy2019_rv.csv'], [2023, 'EFFY2023.csv'], [2024, 'EFFY2024.csv']]) {
  const header = parseCsv(fs.readFileSync(path.join(DIR, file), 'utf8').split(/\r?\n/)[0]);
  const iLev = header.findIndex((h) => unq(h).trim().toUpperCase() === 'EFFYLEV');
  const iTot = header.findIndex((h) => unq(h).trim().toUpperCase() === 'EFYTOTLT');
  let n = 0;
  await eachLine(file, (line) => {
    const c = line.split(',');
    if (parseInt(unq(c[iLev]), 10) !== 2) return; // undergraduate
    rec(unq(c[0]))[`ug12_${year}`] = num(c[iTot]);
    n++;
  });
  console.log(`EFFY${year} undergrad 12mo parsed (${n})`);
}
...
console.log('Sanity Alabama A&M (100654):', JSON.stringify({ name: test.name, state: test.state, control: test.control, comp2024: test.comp['2024'], comp2019: test.comp['2019'], ug_fall_2023: test.ug_fall_2023, ug12_2019: test.ug12_2019, ug12_2024: test.ug12_2024 }));
