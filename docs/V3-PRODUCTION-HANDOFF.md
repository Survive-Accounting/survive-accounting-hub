...
## Update — 2026-09-05: the camera on the slide

Lee: "Previous version of canvas had 'B' … bring that back … make a better frame … 3 fixed positions … a camera off setting … one option for free form … it needs a home spot 70%+ of the time … make sure it never blocks a CEQ, it would probably scale down automatically."

`blastoff/capture/webcam-spots.ts` (pure, tested) + `capture/Webcam.tsx`: the camera belongs to the SLIDE, drawn inside the 9:16 phone, so OBS window-captures it with the slide and the placement repeats. Spots: **home** = a circle bottom-left, its bottom on the caption band (captions go to its right); **corner** = a small circle top-right under the status bar; **hero** = a big rounded portrait top-centre; **free** = drag the ring on the Review stage, wheel to resize (`frame.camPos` / `camSize`); **off**. Defaults: home on every card slide, corner on the intro (the ticker crosses the home spot), off on the open / outro / bolt / ads. Per slide: the Editor's camera chips and the ⋯ menu (`frame.cam`). On the take **B** cycles home → corner → hero → off until the next slide. The Review stage draws a placeholder ring (no permission prompt); the capture asks for the webcam (mirrored, no audio) and fails soft. `avoidCard` measures the live card and shrinks the ring toward its own edge (down to 55 %) so it never sits on a card. The ring: a cream gradient rim, soft glow, navy shadow, "Lee Ingram" nametag under the home circle.

Audio stays OBS's (the mic source); the in-app feed adds ~60–120 ms of webcam latency, so set the mic's sync offset once in OBS (Advanced Audio Properties) after a clap test.

## Update — 2026-09-05: the Survive bolt candidates, the captions tool
...
## Update — 2026-09-05 (overnight): pass 2, the camera moment, the mark lab

**Pass 2** (`blastoff/layout.ts`; picked per set on `/v3/$topic/$set`, saved as `plan.layout`; preview any pass with `localStorage.sa-layout-qa`): every card sits at the TOP of the Shorts safe column (x 5–84 %, y 10–78 %), at 470 flow units × 1.24 (same width, a fifth bigger type, four choices stack portrait); the camera is 30 % of the width at home and a big centred "top" spot above the intro's wordmark (which drops to 44 %); no nametag; spot moves ease with overshoot. **The moment**: ctrl+click the camera on the take — it grows to a centred portrait while the slide dims and blurs; ctrl+click, ` or the next slide ends it. The hand-drawn portrait is parked (on only when a slide says "on").
