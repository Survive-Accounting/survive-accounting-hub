...
- **Build:** `bun run build` — **exit 0** (client + SSR/Vercel output built cleanly). This
  is the real test of the `.functions.ts` server-only-import law; it passed.
- **Typecheck:** `bunx tsc --noEmit` — **0 errors** across the whole repo.
- **Tests:** `bun test src/lib/growth-util.test.ts` — **7 pass / 0 fail**. (One test caught a
  real bug: `councilSlugOf("Panhellenic")` classified as IFC because a loose `"nic"` needle
  matched "panhelle**nic**"; fixed by reordering NPHC-before-Panhellenic and dropping the
  collision-prone needle.)
- **Lint:** new files are prettier-clean; the only remaining eslint findings are
  ...
  formatted.)
- **Dev server:** `/admin/growth` returns HTTP 200; the heaviest read (`getGrowthOverview`,
  which aggregates across all the domain tables) was invoked by the client and returned 200
  against live data. Route tree regenerated by the Vite plugin.
- **verify script:** correctly reports the three `growth_*` tables as not-yet-applied (after
  fixing a false-positive where a HEAD+count request masked the missing-table 404).
